
import pandas as pd
import numpy as np
import mlflow
import mlflow.sklearn
import dagshub
import matplotlib.pyplot as plt
import seaborn as sns
from sklearn.ensemble import RandomForestRegressor
from sklearn.model_selection import TimeSeriesSplit
from sklearn.metrics import mean_squared_error
import os

# Init DagsHub Connection
# GitHub Actions environment variables
DAGSHUB_USERNAME = os.environ.get("DAGSHUB_USERNAME")
DAGSHUB_TOKEN = os.environ.get("DAGSHUB_TOKEN")
DAGSHUB_REPO_NAME = "Eksperimen_SML"

if DAGSHUB_USERNAME and DAGSHUB_TOKEN:
    # Set Tracking URI manually to avoid dagshub.init() auth issues
    os.environ["MLFLOW_TRACKING_USERNAME"] = DAGSHUB_USERNAME
    os.environ["MLFLOW_TRACKING_PASSWORD"] = DAGSHUB_TOKEN
    
    tracking_uri = f"https://dagshub.com/{DAGSHUB_USERNAME}/{DAGSHUB_REPO_NAME}.mlflow"
    mlflow.set_tracking_uri(tracking_uri)
    print(f"MLflow Tracking URI set to: {tracking_uri}")
else:
    print("Warning: DAGSHUB_USERNAME or DAGSHUB_TOKEN not found. Tracking may fail.")

def rmsle(y_true, y_pred):
    return np.sqrt(mean_squared_error(np.log1p(y_true), np.log1p(y_pred)))

def main():
    print("Mulai Training (Basic - No Tuning)...")
    # Load Data
    try:
        # Path disesuaikan untuk struktur submission
        df = pd.read_csv('Membangun_model/Demand_Forecasting_preprocessing/train_processed.csv')
    except FileNotFoundError:
        # Fallback jika dijalankan dari dalam folder Membangun_model
        try:
             df = pd.read_csv('Demand_Forecasting_preprocessing/train_processed.csv')
        except FileNotFoundError:
             print("Data processed tidak ditemukan. Pastikan path benar sesuai struktur submission folder.")
             return

    # Ambil 1 SKU saja untuk demo cepat
    target_sku = 216418
    print(f"Filtering for SKU: {target_sku}")
    
    # Debug: Check avaliable SKUs
    if 'sku_id' in df.columns:
        print(f"Available SKUs in data: {df['sku_id'].unique()}")
        df = df[df['sku_id'] == target_sku]
    
    if df.empty:
        print(f"Error: No data found for SKU {target_sku}!")
        return
    
    # Pilih Fitur
    features = [c for c in df.columns if 'lag_' in c or 'ma_' in c or 'store_encoded' in c or 'total_price' in c]
    X = df[features]
    y = df['units_sold']
    
    mlflow.set_experiment("Retail_Demand_Basic")

    # --- FINAL TRAINING (Tanpa Tuning) ---
    with mlflow.start_run(run_name="Basic_Model"):
        # Parameter tetap (hardcoded)
        params = {
            'n_estimators': 100,
            'max_depth': 10,
            'min_samples_split': 2,
            'random_state': 42
        }
        
        print(f"Training parameters: {params}")
        model = RandomForestRegressor(**params)
        model.fit(X, y)
        
        # 1. Log Model ke DagsHub
        mlflow.sklearn.log_model(model, "model")
        mlflow.log_params(params)
        
        # Hitung Error di Training set (sebagai indikasi saja)
        preds = model.predict(X)
        preds = np.maximum(preds, 0)
        error = rmsle(y, preds)
        mlflow.log_metric("rmsle_train", error)
        print(f"Training RMSLE: {error}")
        
        # 2. Buat Artifact (Grafik)
        plt.figure(figsize=(8, 5))
        pd.Series(model.feature_importances_, index=X.columns).sort_values().plot(kind='barh')
        plt.title("Feature Importance")
        plt.tight_layout()
        plt.savefig("feature_importance.png")
        
        # 3. Upload Artifact ke DagsHub
        mlflow.log_artifact("feature_importance.png")
        print("Model dan Artifact berhasil di-log ke DagsHub!")

if __name__ == "__main__":
    main()
